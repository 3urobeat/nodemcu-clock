...
/**
   \brief Converts Ä --> Any character handed over in the parameter
...
/**
   \brief SPLC780D1 015A
   
   different ROM - more characters
   cyrillic in the upper area
   code duplicated convert_SPLC780D1_002A, convert_SPLC780D1_003A, convert_SPLC780D1_015A
*/
uint8_t convert_SPLC780D1_015A(uint32_t &special, uint8_t &value) {
  //Serial.print("D603 convert_SPLC780D1_015A 0x"); Serial.println(value, HEX);
  uint8_t isWriteable = NOPRINT;
  if ((value & 0b11000000) == 0b11000000)        // start byte
  {
    special = value;
  ...
  else if (isWriteable)      // was UTF-8 2 bytes or more
  {
    ...
    // Lookup with in UTF8->ROM Mapping overrides the character to be send to the LCD
    uint16_t index = 0;
    for (size_t i = 0; i < sizeof(ROM_SPLC780D1_015A) / sizeof(ROM_SPLC780D1_015A[0]); i++)
    {
      index = pgm_read_dword_near(&ROM_SPLC780D1_015A[i].utf8);
      if (index == (special & 0xFFFF))
      {
        memcpy_P(&value, &ROM_SPLC780D1_015A[i].c, 1);
        break;
      }
    }
    // Step 2: Ä Ö Ü overruleUml
...
/**
   \brief Generic converter WITHOUT HW specific, without äöü overrule (as this should be in the ROM table)
   
   needs additional parameters to define the table.
   Does no ROM conversion.
...
/**
  \brief the full ASCII converter
*/
...
/**
   \brief maps UTF-8 characters including up to 8 custom characters
   
   this converter is somehow similar to convert_uml but it relies on 
